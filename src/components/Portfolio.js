...
const projects = [
  ...
  },
  {
    ...
    description: "This project holds a special place as one of my earliest creations. It's a password generator that relies on user input to craft a unique password comprising letters, numbers, and symbols. While my design skills have evolved considerably since then, it's a fun journey back to where it all started. Feel free to give it a try and see how it all began!"
            ...
            {selectedProject ? (
              <div>
                ...
                <h3 className="text-beige text-lg underline flex justify-center mt-6">
                    <a className="pr-6" href={selectedProject.link} target="_blank">Github</a>
                    {selectedProject.video && (
                    <a className="pl-6" href={selectedProject.video} target="_blank">Video Demo</a>
                    )}
                    {selectedProject.sitelink && (
                    <a className="pl-6" href={selectedProject.sitelink} target="">View the Website</a>
                    )}
                </h3>
                </div>
            ...
            <ul className="border border-beige rounded-lg">
              {projects.map((project, index) => (
                <li key={index} className="mb-4">
                  <h3 className="text-xl text-beige font-semibold flex justify-center cursor-pointer p-2" onClick={() => handleProjectClick(index)}>
                    {project.title}
                  </h3>
                </li>
              ))}
